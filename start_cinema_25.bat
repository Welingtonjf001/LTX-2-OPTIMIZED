...
echo Checking port 7961...
for /f "tokens=5" %%P in ('netstat -ano ^| findstr ":7961" ^| findstr "LISTENING"') do (
  echo Stopping old LTX-2.5 CinemaMaker process PID %%P...
  taskkill /PID %%P /T /F >nul 2>&1
)
timeout /t 2 /nobreak >nul

echo LTX-2.5 CinemaMaker: http://127.0.0.1:7961
echo (A primeira geracao sobe o ComfyUI em 8188 e carrega o modelo 2.5: ~1min.)
start "" cmd /c "timeout /t 8 /nobreak >nul && start "" http://127.0.0.1:7961"
".venv\Scripts\python.exe" -u film_maker_ui_v4_25.py
