...
# NOTE: generation goes through gguf_backend (GGUF UNET + distilled LoRA +
# ComfyUI, model chosen via the "GGUF Model" dropdown in the UI), not the fp8
# checkpoint/gemma/upsampler files v2 uses. A scene's Start Image (or an
# auto-chained previous-scene last frame) routes generation through the
# IC-LoRA "Ingredients" workflow, using that image as an identity/appearance
# reference -- NOT literal first-frame video conditioning like the fp8 path.
# See gguf_backend.py docstring for the current known gaps (audio only drives
# scene duration, not generation conditioning; only one reference image per
# scene is used).
AUDIO_CLIPS_DIR = "./audio_clips"
OUTPUT_ROOT = os.path.abspath(os.path.join("outputs", "music_video_gguf"))
...
def v2_status(message, phase=None, progress=None):
    ...
    line = f"[{datetime.datetime.now().strftime('%H:%M:%S')}][V2] {message}"
    ...
    for index in range(1, num_scenes):
        ...
        if phrase_candidates:
            ...
            if word_candidates:
                ...
                # A silent/incomplete ASR window has no legal word boundary.
                # Keep the hard 5-10 s limit and report the fallback to the UI.
                chosen = upper
...
def load_lyrics_segments(audio_path=None, duration_sec=None):
    ...
    # ASR sidecars are saved by transcribe_lyrics_for_audio under a safe stem.
    # Uploaded Gradio files often contain spaces/parentheses, so include that
    # safe name during lookup or a valid transcription is treated as no speech.
...
def get_or_create_vocal_stem(audio_path):
    """Return the Demucs vocal stem used by Wav2Lip, preserving the master mix."""
    global CURRENT_LOG
    ...
    manifest_path = os.path.join(run_subdir("logs"), "vocal_separation.json")

    def write_separation_manifest(device, cached=False):
        manifest = {
            "created_at": datetime.datetime.now().isoformat(),
            "separator": "Demucs 4 / htdemucs",
            "device": device,
            "cached": bool(cached),
            "master_audio_preserved": os.path.abspath(audio_path),
            "wav2lip_vocal_stem": os.path.abspath(vocal_path),
            "instrumental_removed_track": os.path.abspath(instrumental_path),
        }
        with open(manifest_path, "w", encoding="utf-8") as handle:
            json.dump(manifest, handle, ensure_ascii=False, indent=2)

    if os.path.exists(vocal_path) and os.path.exists(instrumental_path):
        write_separation_manifest("cache", cached=True)
        v2_status(
            "usando stems Demucs existentes: vocal para Wav2Lip; original preservado para o MP4 final",
            "Preparando vocal", 85,
        )
        return vocal_path

    # Demucs produces both vocals.wav and no_vocals.wav.  Only vocals.wav is
    # sliced for Wav2Lip; MUSIC_AUDIO_PATH remains the untouched master mix.
    last_error = ""
    ...
    for device in ("cuda", "cpu"):
        if device == "cpu":
            v2_status("Demucs CUDA indisponível; repetindo a separação na CPU", "Preparando vocal", 85)
        else:
            v2_status("Demucs: isolando vocal e removendo instrumental para o Wav2Lip", "Preparando vocal", 85)
        command = [
            sys.executable, "-m", "demucs.separate", "-n", "htdemucs",
            "--two-stems=vocals", "-d", device, "-j", "1",
            "-o", demucs_root, cache_input,
        ]
        try:
            result = subprocess.run(command, cwd=os.getcwd(), capture_output=True, text=True)
        except Exception as exc:
            last_error = f"Demucs ({device}) não iniciou: {exc}"
            v2_status(last_error, "Preparando vocal", 85)
            continue
        terminal_text = (result.stdout or "") + (result.stderr or "")
        if terminal_text:
            CURRENT_LOG += terminal_text[-6000:] + "\n"
            print(terminal_text[-6000:], flush=True)
        if result.returncode == 0 and os.path.exists(vocal_path):
            write_separation_manifest(device)
            v2_status(
                f"Demucs concluído ({device}): vocal pronto para Wav2Lip; faixa original será usada no vídeo final",
                "Preparando vocal", 86,
            )
            return vocal_path
        last_error = terminal_text[-2000:] or f"Demucs terminou com código {result.returncode}"

    CURRENT_LOG += f"Demucs falhou em CUDA e CPU: {last_error}\n"
    v2_status("não foi possível isolar o vocal; Wav2Lip usará o áudio disponível", "Preparando vocal", 86)
    return None
...
def slice_photo_music(audio_path, photo_folder, prompt, fps, num_frames, analyze_clip=True):
    """Build music-video scenes from a photo folder, beat-aligned audio, and CLIP captions."""
    global SCENES_DATA, CURRENT_PHASE, CURRENT_PROGRESS, CURRENT_TOTAL_SCENES, MUSIC_AUDIO_PATH
    if not audio_path:
        return tuple(["Upload the reference music first."] + [gr.update(visible=False)] * 20 * 5)
    v2_status("validando música e pasta de fotos", "Preparação", 1)
    photos = _photo_files(photo_folder)
    if not photos:
        return tuple([f"No supported photos found in: {photo_folder}"] + [gr.update(visible=False)] * 20 * 5)
    try:
        os.makedirs(AUDIO_CLIPS_DIR, exist_ok=True)
        waveform, sr, duration_sec = load_audio_compatible(audio_path)
        MUSIC_AUDIO_PATH = os.path.abspath(audio_path)
        ...
        CURRENT_PHASE = "Analyzing photos and preparing scenes"
        CURRENT_PROGRESS = 2.0
        new_scenes_data = [None] * 20
        ui_updates = []
        ...
        for i in range(20):
            if i < len(photos):
                ui_updates.extend([
                    ...
                    gr.update(value=new_scenes_data[i]["prompt"], visible=True),
                    gr.update(value=new_scenes_data[i]["audio_path"]),
                    gr.update(value=new_scenes_data[i]["photo_path"]),
                    gr.update(value=None),
                ])
            else:
                ui_updates.extend([
        ...
        return tuple([f"Prepared {len(photos)} photo scenes; CLIP {captions_found}/{len(photos)}; {boundary_note}."] + ui_updates)
    except Exception as exc:
        import traceback
        traceback.print_exc()
        return tuple([f"Photo/music slice error: {exc}"] + [gr.update(visible=False)] * 20 * 5)

def process_chain_generation(scenes_list, audios_list, start_images_list, last_images_list, gguf_model, ingredients_lora_strength, steps, fps, width, height, num_frames, seed, random_seed, use_context_compression, latent_reuse_count, context_depth, enable_lipsync=False, face_swap_enabled=False, face_swap_reference=None, face_swap_model="InSwapper 128", face_swap_enhancer="Nenhum", room_preset="none", room_distance=0.0, start_index=None, mode="forward"):
    """
    ...
    for i in indices_to_process:
        ...
        # Generate via the GGUF/ComfyUI backend (persistent server; the 22B
        # model loads once and stays resident across every scene, instead of
        # the fp8 path's per-scene subprocess + full reload).
        try:
            v2_status(
                f"LTX cena {scene_id} (GGUF): iniciando ({actual_num_frames} frames, {width}x{height})",
            ...
            def _scene_log(msg):
                global CURRENT_LOG
                CURRENT_LOG += msg + "\n"
                print(f"[GGUF][cena {scene_id}] {msg}", flush=True)

            ok = gguf_backend.generate_scene(
                prompt=actual_prompt,
                output_path=output_path,
                width=width, height=height, num_frames=int(actual_num_frames),
                fps=fps, steps=int(steps), seed=int(current_seed),
                audio_path=audio_path, conditioning_frames=conditioning_frames,
                gguf=gguf_model, ingredients_lora_strength=float(ingredients_lora_strength),
                log_cb=_scene_log,
                stop_flag_getter=lambda: STOP_GENERATION,
            )
            ...
            if ok and os.path.exists(output_path):
                ...
                # --- Optional post-processing: face swap via roop-unleashed ---
                # Runs AFTER the scene is fully generated (T2V or Ingredients),
                # BEFORE frame extraction/continuity, so downstream scenes chain
                # from the swapped face, not the original one.
                ref_path = get_valid_path(face_swap_reference)
                if face_swap_enabled and ref_path:
                    swapped_path = os.path.abspath(os.path.join(
                        run_subdir("scenes"), f"scene_{scene_id:02d}_{timestamp}_faceswap.mp4"
                    ))
                    enhancer_arg = None if face_swap_enhancer in (None, "Nenhum") else face_swap_enhancer
                    CURRENT_LOG += f"Aplicando face swap (roop) na cena {scene_id}...\n"

                    def _swap_log(msg):
                        global CURRENT_LOG
                        CURRENT_LOG += msg + "\n"
                        print(f"[ROOP][cena {scene_id}] {msg}", flush=True)

                    swap_ok = roop_backend.swap_face(
                        source_face_path=ref_path,
                        target_video_path=output_path,
                        output_path=swapped_path,
                        swap_model=face_swap_model,
                        enhancer=enhancer_arg,
                        log_cb=_swap_log,
                    )
                    if swap_ok and os.path.exists(swapped_path):
                        output_path = swapped_path
                        LATEST_VIDEO_PATH = output_path
                        CURRENT_LOG += f"Face swap concluído: {output_path}\n"
                    else:
                        CURRENT_LOG += f"Face swap falhou na cena {scene_id}; mantendo vídeo sem troca de rosto.\n"
    ...
    if mode == "forward" and not STOP_GENERATION:
        ...
        if paths_ready:
            try:
                ...
                if enable_lipsync:
                    CURRENT_PHASE = "Lip-sync per scene (Wav2Lip)"
                    ...
                    for position, scene_index in enumerate(valid_indices):
                        ...
                        lip_output = os.path.abspath(os.path.join(
                            run_subdir("lipsync"), f"scene_{scene_index + 1:02d}_{stamp}_lipsync.mp4"
                        ))
                        synced = run_wav2lip(paths_for_concat[position], lip_audio, lip_output)
                        if synced:
    ...
    if CURRENT_RUN_DIR:
        try:
            ...
            print(f"[V2] Could not save run log: {log_error}", flush=True)
    IS_PROCESSING = False

def start_generation_thread(prompts, audios, start_images, last_images, gguf_model, ingredients_lora_strength, steps, fps, width, height, num_frames, seed, random_seed, use_context_compression, latent_reuse_count, context_depth, enable_lipsync=False, torch_compile_enabled=False, ambient_enabled=False, ambient_volume=0.35, face_swap_enabled=False, face_swap_reference=None, face_swap_model="InSwapper 128", face_swap_enhancer="Nenhum", room_preset="none", room_distance=0.0, start_index=None, mode="forward"):
    ...
    threading.Thread(target=process_chain_generation, args=(prompts, audios, start_images, last_images, gguf_model, ingredients_lora_strength, steps, fps, width, height, num_frames, seed, random_seed, use_context_compression, latent_reuse_count, context_depth, enable_lipsync, face_swap_enabled, face_swap_reference, face_swap_model, face_swap_enhancer, room_preset, room_distance, start_index, mode), daemon=True).start()
...
with gr.Blocks(title="LTX-2 Music Video Maker (GGUF/ComfyUI)") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            enable_lipsync = gr.Checkbox(
                ...
                info="Runs Wav2Lip per singing scene with its exact vocal segment, then restores the original music; requires wav2lip_gan.pth and s3fd.pth.",
                value=False,
            )
            torch_compile_enabled = gr.Checkbox(
                label="torch.compile (native backend only)", value=False, interactive=False,
                info="The GGUF/ComfyUI workflow does not currently wire a compile node.",
            )
            ...
            room_preset = gr.Dropdown(
                ...
                info="Aplicado no vocal isolado DEPOIS do Wav2Lip (sincronia labial usa sempre a voz seca); dá naturalidade/espaço à fala do personagem sem afetar o timing.",
            )
            ...
            with gr.Accordion("LTX-2 Settings (GGUF)", open=True):
                gguf_model = gr.Dropdown(
                    label="GGUF Model",
                    choices=gguf_backend.list_available_gguf(),
                    value=gguf_backend.DEFAULT_GGUF,
                    info="Q6_K = melhor qualidade, mais lento. UD-Q5_K_S = mais leve/rápido.",
                )
                ingredients_lora_strength = gr.Slider(
                    label="Reference Face Strength (IC-LoRA Ingredients)",
                    minimum=0.0, maximum=1.0, step=0.05,
                    value=gguf_backend.DEFAULT_INGREDIENTS_LORA_STRENGTH,
                    info="Só é usado quando a cena tem uma Start Image: controla o quanto o rosto/identidade "
                         "da imagem de referência domina a geração via IC-LoRA Ingredients. "
                         "1.0 = validado (reprodução bem próxima da referência, prompt tem pouco efeito na aparência). "
                         "Valores menores ainda não foram testados por nós -- ajuste e observe o resultado.",
                )
                with gr.Row():
            ...
            with gr.Accordion("Face Swap (roop, pós-processamento)", open=False):
                face_swap_enabled = gr.Checkbox(
                    label="Enable Face Swap (roop)",
                    value=False,
                    info="Após cada cena ser gerada (T2V ou Ingredients), troca o rosto pelo da imagem de "
                         "referência abaixo via roop-unleashed, rodando em subprocess isolado (venv próprio "
                         "do roop, GPU dedicada). Independente do 'Reference Face Strength' acima (que é a "
                         "identidade via IC-LoRA na própria geração LTX) -- pode usar os dois juntos ou só um.",
                )
                face_swap_reference = gr.Image(label="Face Reference Image", type="filepath")
                with gr.Row():
                    face_swap_model = gr.Dropdown(
                        label="Swap Model",
                        choices=["InSwapper 128", "ReSwapper 128", "ReSwapper 256"],
                        value=roop_backend.DEFAULT_SWAP_MODEL,
                    )
                    face_swap_enhancer = gr.Dropdown(
                        label="Enhancer (opcional)",
                        choices=["Nenhum", "GFPGAN", "Codeformer", "DMDNet", "GPEN", "Restoreformer++"],
                        value="Nenhum",
                    )
    ...
    def collect_prompts_and_start(*args):
        # Args structure: [prompts_list..., audios..., start_img..., last_img..., gguf_model, ..., button_args]
    ...
    all_inputs = scene_prompts + scene_audios + scene_start_images + scene_last_images + [gguf_model, ingredients_lora_strength, steps, fps, width, height, num_frames, seed, random_seed, use_context_compression, latent_reuse_count, context_depth, enable_lipsync, torch_compile_enabled, ambient_enabled, ambient_volume, face_swap_enabled, face_swap_reference, face_swap_model, face_swap_enhancer, room_preset, room_distance]
    ...
    photo_slice_btn.click(
        ...
        outputs=[photo_status] + [comp for tuple_5 in zip(scene_rows, scene_prompts, scene_audios, scene_start_images, scene_last_images) for comp in tuple_5]
    )
