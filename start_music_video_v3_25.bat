@echo off
setlocal
title LTX-2.5 Music Video Maker V3
...
echo Checking port 7904...
for /f "tokens=5" %%P in ('netstat -ano ^| findstr ":7904" ^| findstr "LISTENING"') do (
  echo Stopping old LTX-2.5 Music Video V3 process PID %%P...
  taskkill /PID %%P /T /F >nul 2>&1
)
timeout /t 2 /nobreak >nul

echo LTX-2.5 Music Video V3: http://127.0.0.1:7904
echo (A primeira geracao sobe o ComfyUI em 8188 e carrega o modelo 2.5: ~1min.)
start "" cmd /c "timeout /t 8 /nobreak >nul && start "" http://127.0.0.1:7904"
".venv\Scripts\python.exe" -u music_maker_ui_v3_25.py
