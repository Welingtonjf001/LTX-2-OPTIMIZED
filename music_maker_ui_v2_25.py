"""LTX-2.5 variant of music_maker_ui_v2.py -- GENERATED by _make_25_uis.py, do not hand-edit
...
Differences from the 2.3 original:
  ...
  - Gradio port moved to 7903 so this can run alongside the 2.3 UI.
...
# Modelo do upscale final (Real-ESRGAN). MEDIDO 2026-08-24 num render 3D estilo
# Pixar, ampliando o MESMO quadro por caminhos diferentes: o anime-v3 -- que era
# o unico usado aqui -- ACHATA o rosto em blocos de cor chapada (olhos viram
# traco preto, o brinco de esmeralda some), enquanto o x4plus preserva o volume.
# Ele pontua mais em "nitidez" (Laplacian 245 contra 201) justamente porque troca
# gradiente por borda dura: a metrica mede contraste, nao correcao.
# A 1280x704 a diferenca deixa de ser gritante, por isso o padrao segue o de
# antes -- mas a escolha agora e do usuario. Ver MEMORIAL.md secao 3.18.
# Cada preset amarra modelo E escala: o x4plus so existe em 4x, o anime-v3 em
# 2x/3x/4x (o executavel resolve <nome>-x<escala>.bin).
UPSCALE_PRESETS = {
    "Anime 3x (realesr-animevideov3) - padrao": ("realesr-animevideov3", 3),
    "3D/Foto 4x (realesrgan-x4plus)": ("realesrgan-x4plus", 4),
    "Anime 4x (realesrgan-x4plus-anime)": ("realesrgan-x4plus-anime", 4),
    "Sem upscale": ("", 0),
}
DEFAULT_UPSCALE_PRESET = "Anime 3x (realesr-animevideov3) - padrao"
    ...
    if mode == "forward" and not STOP_GENERATION:
        ...
        if paths_ready:
            try:
                ...
                # Modelo e escala vêm do seletor da UI via ambiente, no mesmo
                # padrão de LTX_TORCH_COMPILE/LTX_AMBIENT_*. Modelo vazio =
                # "Sem upscale": entrega o vídeo remuxado como final.
                up_model = os.environ.get("LTX_UPSCALE_MODEL", "realesr-animevideov3")
                up_scale = os.environ.get("LTX_UPSCALE_SCALE", "3")
                if not up_model or up_scale in ("", "0"):
                    v2_status("upscale desativado no seletor; usando o vídeo na resolução nativa",
                              "Áudio final", 98)
                    CURRENT_LOG += "Upscale skipped (selector set to none).\n"
                    LATEST_VIDEO_PATH = final_video
                else:
                    CURRENT_PHASE = f"Upscaling final video {up_scale}x"
                    CURRENT_PROGRESS = 96.0
                    v2_status(f"iniciando upscale {up_model} {up_scale}x", CURRENT_PHASE, CURRENT_PROGRESS)
                    CURRENT_LOG += f"Applying Real-ESRGAN upscale {up_model} {up_scale}x...\n"
                ...
                # LATEST_VIDEO_PATH, não upscaled_video: no caminho "Sem upscale"
                # esse arquivo nunca é criado, e anunciá-lo daria um caminho morto.
                v2_status(f"vídeo final pronto: {LATEST_VIDEO_PATH}", CURRENT_PHASE, CURRENT_PROGRESS)
                CURRENT_LOG += f"Final video ready: {LATEST_VIDEO_PATH}\n"
...
def start_generation_thread(prompts, audios, start_images, last_images, checkpoint, gemma, upsampler, steps, fps, width, height, num_frames, seed, random_seed, use_context_compression, latent_reuse_count, context_depth, enable_lipsync=False, torch_compile_enabled=False, ambient_enabled=False, ambient_volume=0.35, room_preset="none", room_distance=0.0, upscale_preset=DEFAULT_UPSCALE_PRESET, start_index=None, mode="forward"):
    ...
    _up_model, _up_scale = UPSCALE_PRESETS.get(upscale_preset, UPSCALE_PRESETS[DEFAULT_UPSCALE_PRESET])
    os.environ["LTX_UPSCALE_MODEL"] = _up_model
    os.environ["LTX_UPSCALE_SCALE"] = str(_up_scale)
...
with gr.Blocks(title="LTX-2.5 Music Video Maker V2") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            upscale_preset = gr.Dropdown(
                choices=list(UPSCALE_PRESETS),
                value=DEFAULT_UPSCALE_PRESET,
                label="Upscale final (Real-ESRGAN)",
                info="anime-v3 afia demais e achata render 3D; x4plus preserva volume. "
                     "Escolha 'Sem upscale' se o clipe ja sai na resolucao final.",
            )
